...
export default function HelpContent() {
  ...
  const topic = params.topic as string | string[] | undefined
  ...
  const validTopic = topicStr ? helpTopics[topicStr] : undefined
  const showNotFound = topicStr && !validTopic
            ...
            <ul style={{listStyle: 'none', padding: 0, marginTop: 10}}>
              {links.map((l) => (
                <li key={l.slug} style={{marginBottom: 2}}>
                  <Link href={l.slug ? `/help/${l.slug}` : '/help'}
                    className="scroll-reveal"
                    style={{
                      display: 'block', padding: '7px 12px', fontSize: 13, textDecoration: 'none',
                      borderRadius: 3, transition: 'all 0.15s',
                      background: (!topicStr && !l.slug) || topicStr === l.slug ? '#e6f5ec' : 'transparent',
                      color: (!topicStr && !l.slug) || topicStr === l.slug ? '#005334' : '#555',
                      fontWeight: (!topicStr && !l.slug) || topicStr === l.slug ? 'bold' : 'normal'
                    }}>
                    {l.label}
                  </Link>
                </li>
          ...
          {showNotFound ? (
            <div className="anim-scale-in" style={{background: '#fff', border: '1px solid #eee', padding: 24, textAlign: 'center'}}>
              ...
              <p style={{color: '#666', marginBottom: 12}}>
                The help topic &quot;{topicStr}&quot; doesn&apos;t exist. Please choose from the topics on the left.
              </p>
              <Link href="/help" style={{color: '#005334', textDecoration: 'underline', fontSize: 13}}>Back to Help Center</Link>
            </div>
          ) : (
